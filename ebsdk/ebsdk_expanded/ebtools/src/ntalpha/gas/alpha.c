    ...
    relax_addressT segment_address_in_file;
{
    ...
    long add_number;
    char ri_emit[RELOCATION_INFO_SIZE];
...
/*	    md_ri_to_chars((char *) &ri, ri);        */
	    ...
	    /* Copy the whole structure, element by element, to a 
	     * character array to ensure correct allignment (only realy
	     * needed for NT but safe for all OSs so do it anyway)
	     */
	    bcopy((char *) &ri.r_address, ri_emit+R_ADDRESS_OFFSET, R_ADDRESS_SIZE);
	    bcopy((char *) &ri.r_index, ri_emit+R_INDEX_OFFSET, R_INDEX_SIZE);
	    bcopy((char *) &ri.r_extern, ri_emit+R_EXTERN_OFFSET, R_EXTERN_SIZE);
	    /* Can't take the address of a field member so copy it by
	       assignment */
	    ri_emit[R_TYPE_OFFSET] = (char) ri.r_type;
	    bcopy((char *) &ri.r_addend, ri_emit+R_ADDEND_OFFSET, R_ADDEND_SIZE);
	    append(&next_object_file_charP, ri_emit, RELOCATION_INFO_SIZE);
	}
    }
    return;
}
