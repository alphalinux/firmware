...
#define index(string,char) strchr(string,char)
#endif
     ...
     int pre;
{
	...
	if (docpp)
	{
		...
		int s;
#ifdef NT
		(void)strcpy (temporary_file_name, "apXXXXXX");
#else		
		...
		if(f_out==(FILE *)0) {
			as_perror("Can't open temp file", temporary_file_name);
		}
#ifdef CMUCS
...
#ifdef NT
		  fclose(f_out);
		  cmd = malloc(sizeof("cl /EP /Zs /D__GNU_AS__=1 ") + 
			       strlen(filename) + 
			       sizeof(" > ") + 
			       strlen(temporary_file_name) + 2);
		  sprintf(cmd, "cl /EP /Zs /D__GNU_AS__=1 %s > %s", filename, temporary_file_name);
#else
...
#endif
		if (s) {
			as_perror("Could not run cpp", "cl");
			return;
		}
#ifdef NT
		f_in=fopen(temporary_file_name,"r");
