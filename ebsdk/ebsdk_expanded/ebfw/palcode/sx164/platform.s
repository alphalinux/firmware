/*
 *      VID: [2.0] PT: [Thu Aug 21 16:25:37 1997] SF: [platform.s]
...
**
**  $Id: platform.s,v 1.1.1.1 1998/12/29 21:36:08 paradis Exp $
** 
...
#if !defined(lint)
	.data
	.asciz "$Id: platform.s,v 1.1.1.1 1998/12/29 21:36:08 paradis Exp $"
#endif
...
/* SX164 */
/*
** Disable Machine Checks for now.  Machine Check handlers have
** not been setup yet.
**
...
**      p1 () - MCR Address
*/
	LDLI(p4, (BC_M_IO_PAR_ENA | BC_M_MEM_PAR_ENA))		// Load up Mck Bits.
	bic	t0, p4, a0				// Disable them for now.
	SAVE_SHADOW(a0,CNS_Q_BC_CFG2,p5)	// Save shadow of bcCfg2.

	ldah	v0, 0xFFF0(zero)	// v0 <- FFFF.FFFF.FFF0.0000
	zap	v0, 0xE0, v0		// Get base address of FF.FFF0.0000
	bis	t1, zero, a1		// Load up current BC_CONFIG
...
pvc$osf81$5080:
// RBB disable for now -	bsr	p6, Wr_Bcache		// Call cache-block-aligned routine
...
Sys_Cserve:
	...
	cmpeq	a2, CSERVE_K_RD_BCCFG2, v0
	bne	v0, Sys_Cserve_Rd_bcCfg2
	...
	cmpeq	a2, CSERVE_K_RD_MAF_MODE, v0
	bne	v0, Sys_Cserve_Rd_MafMode

	cmpeq	a2, CSERVE_K_WR_MAF_MODE, v0
	bne	v0, Sys_Cserve_Wr_MafMode
...
*/
	RESTORE_SHADOW(t0,CNS_Q_BC_CFG2,t9)	// Get bcCfg2.
...
Sys_Cserve_Rd_MafMode:
        mfpr	v0, mafMode			// Read MAF_MODE.
	hw_rei

/*
**
** FUNCTIONAL DESCRIPTION:
**
**	Write the Miss Address Mode Register (MAF_MODE) IPR.
**
** INPUT PARAMETERS:
**
**      a0 (r16) - New MAF_MODE value
**
** OUTPUT PARAMETERS:
**
**	None
...
Sys_Cserve_Wr_MafMode:
	mtpr	a0, mafMode		// Write MAF_MODE
...
** FUNCTIONAL DESCRIPTION:
**
**	Return the shadow copy of the BC_CONFIG2 IPR in 
**      register v0 (r0).
**
** INPUT PARAMETERS:
**
...
** OUTPUT PARAMETERS:
**
**	v0 (r0) - Returned BC_CONFIG2 value
**
** SIDE EFFECTS:
**
*/
...
pvc$osf80$5080:
	...
	SAVE_SHADOW(a0,CNS_Q_BC_CFG2,p0)	// Save shadow copy
...
1:	subq	p4, 1, p4		// Decrement the counter
	...
	mb
	stq_p	a0, bcCfg2(v0)		// Write the control
	mb
2:	bgt	p4, 3f
	stq_p	a1, bcCfg(v0)	// Write the configuration
	mb
	stl_p	a3, 0(p1)		// Write MCR
	mb
	ldl_p	a3, 0(p1)
3:	bgt	p4, 4f
	mb
	stl_p	a4, 0(p2)		// Write GTR
	mb
	ldl_p	a4, 0(p2)
